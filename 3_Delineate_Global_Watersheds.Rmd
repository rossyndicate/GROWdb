...
```{r}
# ---------------------------------------------------------------------------
# wrong flowline selected: 5
# ---------------------------------------------------------------------------
r5_flowline <- select_flowline_global(sites = intl_unique %>% filter(rowid == 5),
                                      flowlines = hydro_rivers_review,
                                      river_buffer_m = 10000,
                                      id_col = "rowid")

delineate_ws(sites = intl_unique %>% filter(rowid == 5),
             flowline_selections = r5_flowline,
             flowlines_network = hydro_rivers,
             basins = basins,
             flow_acc_raster = terra::rast("data/hyd_glo_acc_15s/hyd_glo_acc_15s.tif"),
             flow_dir_raster = terra::rast("data/hyd_glo_dir_15s/hyd_glo_dir_15s.tif"),
...
mapview(readRDS("data/intl_watersheds_refined/5.RDS")) + intl_unique %>% filter(rowid == 5)

# ---------------------------------------------------------------------------
# wrong flowline selected: 14
# ---------------------------------------------------------------------------
r14_flowline <- select_flowline_global(sites = intl_unique %>% filter(rowid == 14),
                                       flowlines = hydro_rivers_review,
                                       river_buffer_m = 10000,
                                       id_col = "rowid")

watersheds <- delineate_ws(sites = intl_unique %>% filter(rowid == 14),
                           flowline_selections = r14_flowline,
                           flowlines_network   = hydro_rivers,
                           basins = basins,
...
mapview(readRDS("data/intl_watersheds_refined/14.RDS")) + intl_unique %>% filter(rowid == 14)


# ---------------------------------------------------------------------------
# wrong flowline selected: 41
# ---------------------------------------------------------------------------
r41_flowline <- select_flowline_global(sites = intl_unique %>% filter(rowid == 41),
                                       flowlines = hydro_rivers_review,
                                       river_buffer_m = 10000,
                                       id_col = "rowid")

watersheds <- delineate_ws(sites = intl_unique %>% filter(rowid == 41),
...
#. Create final watershed sf object:
